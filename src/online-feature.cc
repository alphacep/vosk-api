...
const float *RecyclingVector::At(int32_t index) const {
  if (index < first_available_index_) {
    SHERPA_ONNX_LOG(FATAL) << "Attempted to retrieve feature vector that was "
...
void OnlineGenericBaseFeature<C>::AcceptWaveform(float sampling_rate,
  ...
  if (input_finished_) {
    SHERPA_ONNX_LOG(FATAL) << "AcceptWaveform called after InputFinished() was called.";
  }

  SHERPA_ONNX_CHECK_EQ(sampling_rate, computer_.GetFrameOptions().samp_freq);
...
void OnlineGenericBaseFeature<C>::ComputeFeatures() {
  ...
  SHERPA_ONNX_CHECK_GE(num_frames_new, num_frames_old);
